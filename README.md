...
### Managing Outdated Files

To keep the `src/ai_service` directory clean and manageable, you can use the `delete_outdated_ai_service.py` script to remove files that haven't been modified in a specified period.

#### Usage

1.  Navigate to the project root directory.

2.  Run the script with the directory and number of days as arguments:

    ```bash
    python src/delete_outdated_ai_service.py src/ai_service 30
    ```

    This command will delete files in the `src/ai_service` directory that haven't been modified in the last 30 days.

#### Explanation

*   `src/delete_outdated_ai_service.py`: The script that deletes outdated files.
*   `src/ai_service`: The directory to clean.
*   `30`: The number of days after which a file is considered outdated.
This script helps maintain the project by removing old and unused files, ensuring that the codebase remains relevant and efficient.### Deleting the ai-service/ai_service DirectoryAfter consolidating the contents of `/workspaces/vscode-platform/src/ai_service` and `/workspaces/vscode-platform/ai-service/ai_service`, you can use the `delete_outdated_ai_service.py` script to delete the now-empty directory.#### Usage1.  Navigate to the project root directory.2.  Run the script:    ```bash    python src/delete_outdated_ai_service.py    ```    The script will prompt you to confirm the deletion of the `/workspaces/vscode-platform/ai-service/ai_service` directory. Type `y` to confirm or `n` to cancel.#### Explanation*   `src/delete_outdated_ai_service.py`: The script that deletes the specified directory.**Important:** Ensure that you have consolidated the necessary files before running this script, as it will permanently delete the directory.

### License Management

SupercoderAI includes a license management system to handle user subscriptions and access.

#### Generating a License Key

To generate a license key for a user, use the `license_manager.py` script:

1.  Navigate to the project root directory.

2.  Run the script with the user ID:

    ```bash
    python src/license_manager.py
    ```

    This will generate a license key for the specified user ID.

#### Validating a License Key

To validate a license key for a user, use the `validate_license_key` method in the `LicenseManager` class:
...
is_valid = license_manager.validate_license_key(user_id, license_key)
print(f"Is License Key Valid? {is_valid}")
```

**Important:** Replace `your_secret_key` with a secure secret key for your application.
...
2.  Run the script with the user ID:
    ...
    This will generate a license key for the specified user ID.
#### Validating a License KeyTo validate a license key for a user, use the `validate_license_key` method in the `LicenseManager` class:```pythonfrom src.license_manager import LicenseManagersecret_key = "your_secret_key"user_id = "user123"license_key = "generated_license_key"license_manager = LicenseManager(secret_key)
is_valid = license_manager.validate_license_key(user_id, license_key)print(f"Is License Key Valid? {is_valid}")```
**Important:** Replace `your_secret_key` with a secure secret key for your application.## Contributing
